...
export async function POST(request: Request) {
  try {
    ...
    const now = new Date(); const user = { name: name.trim(), email: normalizedEmail, password: await hashPassword(password), role: "CUSTOMER", createdAt: now, updatedAt: now };
    ...
    response.cookies.set(sessionCookie(createSessionToken({ id: savedUser.id, name: user.name, email: user.email, role: "CUSTOMER" })));
    return response;
